...
web-smoke:
	...
	umask 077; printf '%s\n' 'smoke-token-0123456789abcdef0123456789abcdef' > "$$tmp_dir/web.token"; \
	NEONEXUS_DATA_DIR="$$tmp_dir" cargo run -- --web --port 8712 --web-token-file "$$tmp_dir/web.token" > "$$tmp_dir/web.log" 2>&1 & \
	server_pid=$$!; \
...
# Full wire contract against a disposable Rust signer service. The sibling
# service-owned harness builds, starts, provisions, and always removes its vault.
signer-compat:
	bash scripts/run-signer-contract.sh
