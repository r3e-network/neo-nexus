...
- CLI parsing for GUI default, version/help output, self-check, runtime-smoke,
  ...
  rejection.
- Manager mode planning for default GUI launch, explicit CLI output, CLI exit
  code preservation, and the binary entrypoint's use of the manager layer
  instead of direct CLI action matching.
- Core facade coverage proving reusable domain services are available without
  depending on GUI application-shell modules.
