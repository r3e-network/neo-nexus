...
- Dual-mode application boundary: startup arguments are classified by a
  ...
  readiness, metrics, runtime catalogs, alerts, federation, config rendering,
  logs, snapshots, wallets, private-network plans, node-state types, and
  formatting. That keeps the primary operator surfaces, text/JSON automation
  paths, and app flows on the same tested boundary instead of letting them
  depend directly on lower-level runtime, model, or service modules.
