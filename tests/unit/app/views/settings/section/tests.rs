...
fn every_section_round_trips_through_its_persist_key() {
    for section in SettingsSection::ALL {
        ...
        assert_eq!(
            SettingsSection::from_persist_key(section.persist_key()),
            Some(section),
        );
    }
    assert_eq!(SettingsSection::from_persist_key("not-a-section"), None);
}

#[test]
fn persist_keys_are_unique() {
    let mut keys: Vec<_> = SettingsSection::ALL
        .iter()
        .map(|s| s.persist_key())
        .collect();
    keys.sort_unstable();
    keys.dedup();
    assert_eq!(keys.len(), SettingsSection::ALL.len());
}
