use super::*;
use crate::{app::RemoteFederationProbeResult, federation::RemoteServerProbeReport};
...
fn remote_federation_drain_clears_pending_for_a_profile_deleted_mid_probe() -> anyhow::Result<()> {
    ...
    app.remote_federation_pending.insert(profile.id.clone());
    app.remote_federation_sender
    ...
    assert!(!app.remote_federation_pending.contains(&profile.id));
    assert!(app
        .repository
