...
#[test]
fn node_port_validation_rejects_zero_p2p_and_duplicate_ports() {
...
#[test]
fn node_port_validation_accepts_disabled_rpc_port_zero() {
    assert!(validate_node_ports(0, 10333, None).is_ok());
    assert!(validate_node_ports(0, 10333, Some(10334)).is_ok());
    assert!(validate_node_ports(0, 0, None).is_err());
}
