...
pub(super) fn assert_managed_configs(workspace_root: &Path, created: &[NodeConfig]) {
    ...
    // The committee is a protocol setting, not a per-node config key.
    assert!(validator_config["consensus"].get("validators").is_none());
    ...
    assert!(rpc_config["consensus"].get("validators").is_none());
}
