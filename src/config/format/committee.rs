//! The public standby committees for Neo N3 mainnet and testnet.
//!
//! neo-go refuses to start without `StandbyCommittee`
//! ("configuration should include StandbyCommittee") and refuses again if it
//! holds fewer keys than `ValidatorsCount`, so a generated public-network
//! config that omits these is unbootable. They are consensus-critical constants
//! of the network, identical across clients, and are transcribed verbatim from
//! nspcc-dev/neo-go `config/protocol.{mainnet,testnet}.yml` — which carry the
//! same lists as the C# node's own configuration.
//!
//! A private network supplies its own committee through
//! [`super::RuntimeConfigProfile`]; these are only the fallback for the two
//! public networks.

use crate::types::Network;

/// Committee size on both public networks. Guards the transcription: a list
/// that lost or gained a key stops being the network's committee.
pub(in crate::config) const PUBLIC_COMMITTEE_SIZE: usize = 21;

const MAINNET_COMMITTEE: [&str; PUBLIC_COMMITTEE_SIZE] = [
...
const TESTNET_COMMITTEE: [&str; PUBLIC_COMMITTEE_SIZE] = [
...
/// The standby committee for a public network, or an empty list for a private
/// one, whose committee comes from its own profile.
pub(in crate::config) fn standby_committee(network: Network) -> Vec<String> {
    match network {
        Network::Mainnet => MAINNET_COMMITTEE
            .iter()
            .map(|key| (*key).to_string())
            .collect(),
        Network::Testnet => TESTNET_COMMITTEE
            .iter()
            .map(|key| (*key).to_string())
            .collect(),
...
#[cfg(test)]
#[path = "../../../tests/unit/config/format/committee/tests.rs"]
mod tests;
