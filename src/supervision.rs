...
impl LoopState {
    ...
    fn probe_runtime_upgrade(&mut self, state: &EngineState) {
        // 1. 加载 policy
        let Ok(policy) = state.repository.load_runtime_upgrade_policy() else {
            eprintln!("neo-nexus: failed to load runtime upgrade policy");
            return;
        };
        ...
        let Ok(nodes) = state.repository.list_nodes() else {
            eprintln!("neo-nexus: failed to list nodes for upgrade check");
            return;
        };
        ...
        let Some(profile_id) = policy.catalog_profile_id.as_deref() else {
            eprintln!("neo-nexus: catalog profile ID not configured");
            let updated_policy = policy.with_checked_at(now_unix);
            let _ = state
                .repository
                .save_runtime_upgrade_policy(&updated_policy);
        ...
        let Ok(profiles) = state.repository.list_runtime_catalog_profiles() else {
            eprintln!("neo-nexus: failed to list catalog profiles");
            let updated_policy = policy.with_checked_at(now_unix);
            let _ = state
                .repository
                .save_runtime_upgrade_policy(&updated_policy);
        ...
        let Some(profile) = profiles.iter().find(|p| p.id == *profile_id) else {
            eprintln!("neo-nexus: catalog profile {} not found", profile_id);
            let updated_policy = policy.with_checked_at(now_unix);
            let _ = state
                .repository
                .save_runtime_upgrade_policy(&updated_policy);
        ...
        let all_nodes_map: std::collections::HashMap<String, NodeConfig> =
        ...
        for candidate in plan.into_ready_candidates() {
            ...
            let node = match all_nodes_map.get(&node_id) {
                Some(n) => n,
                None => {
                    eprintln!("neo-nexus: node {} not found in lookup", node_name);
                    continue;
                }
            };
                ...
                Ok(download) => {
                    ...
                    match RuntimePackageManager::install(&manifest, &install_root) {
                        Ok(installation) => {
                            ...
                            let new_node = crate::types::NewNode {
                                ...
                                Ok(_updated) => {
                                    ...
                                    if was_running_before_upgrade {
                                        match launch_node(state, node, LaunchAction::Start) {
                                            Ok(msg) => {
                                                eprintln!("neo-nexus: {} upgraded from {from_ver} to {to_ver}: {msg}", node_name);
                                                success_count += 1;
                                            }
                                            Err(e) => {
                                                eprintln!("neo-nexus: failed to start {} after upgrade: {e}", node_name);
                                            }
                                        }
                                    } else {
                                        eprintln!("neo-nexus: {} upgraded from {from_ver} to {to_ver} (not restarting)", node_name);
                                        success_count += 1;
                                    }
                                }
                                Err(e) => {
