...
impl NeoNexusApp {
    pub(in crate::app) fn save_rpc_health_monitor_policy(&mut self) {
        ...
        let policy = self.async_bus.rpc_health_monitor_policy_draft.to_policy();
        match self.repository.save_rpc_health_monitor_policy(policy) {
            Ok(()) => {
                ...
                let message = format!(
                    ...
                    self.async_bus.rpc_health_monitor_policy.describe()
                );
    ...
    pub(in crate::app) fn save_remote_federation_monitor_policy(&mut self) {
        ...
        {
            Ok(()) => {
                self.async_bus.remote_federation_monitor_policy = policy.normalized();
                ...
                let message = format!(
                    ...
                    self.async_bus.remote_federation_monitor_policy.describe()
                );
                self.record_event_notice(
