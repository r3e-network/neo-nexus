...
impl NeoNexusApp {
    pub(super) fn render_port_matrix(&mut self, ui: &mut egui::Ui, diagnostics: &FleetDiagnostics) {
        ...
        self.operations_ui.port_matrix_page = self.operations_ui.port_matrix_page.min(total_pages - 1);
        pagination_bar(ui, &mut self.operations_ui.port_matrix_page, total_pages, rows.len());
        ui.separator();
