...
pub(super) fn render_rpc_monitor_policy(app: &mut NeoNexusApp, ui: &mut egui::Ui) {
    ui.checkbox(
        &mut app.rpc_health_monitor_policy_draft.enabled,
        "Automatically check RPC health for running nodes",
    );
    interval_drag(
        ui,
        "RPC interval",
        &mut app.rpc_health_monitor_policy_draft.interval_seconds,
        RpcHealthMonitorPolicy::MIN_INTERVAL_SECONDS..=RpcHealthMonitorPolicy::MAX_INTERVAL_SECONDS,
        5.0,
    );

    validation_error(ui, app.rpc_health_monitor_policy_draft.validation_message());
...
fn render_rpc_actions(app: &mut NeoNexusApp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        let can_save = app
            ...
            && app
                .rpc_health_monitor_policy_draft
                .differs_from(app.rpc_health_monitor_policy);
        if ui
        ...
        if ui
            .add_enabled(
                app.rpc_health_monitor_policy_draft
                    .differs_from(app.rpc_health_monitor_policy),
