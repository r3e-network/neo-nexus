...
const USAGE_LINES: &[&str] = &[
    ...
    "  neo-nexus --rpc-health <port|url> [neo-n3|neo-x]",
    "  neo-nexus --rpc-health-json <port|url> [neo-n3|neo-x]",
    ...
    "  neo-nexus --config-drift <neonexus.db>",
    "  neo-nexus --config-drift-json <neonexus.db>",
...
const OPTION_LINES: &[&str] = &[
    ...
    "  --rpc-health                 Check a Neo N3 or Neo X JSON-RPC endpoint (append neo-x)",
    "  --rpc-health-json            Print N3 or Neo X health probe evidence as JSON",
    ...
    "  --config-drift               Compare on-disk managed configs with a fresh workspace render",
    "  --config-drift-json          Print config drift findings as JSON; exits 1 when a node needs attention",
