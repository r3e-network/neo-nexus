...
pub(in crate::cli) fn action_from_args_vec(args: &[String]) -> Result<CliAction> {
    ...
    match first.as_str() {
        ...
        "--designation" => Ok(CliAction::Print(designation_text(args)?)),
        "--designation-json" => designation_json_action(args),
        "--governance" => Ok(CliAction::Print(governance_text(args)?)),
        "--governance-json" => governance_json_action(args),
        "--rpc-health" => Ok(CliAction::Print(rpc_health_text(args)?)),
        "--rpc-health-json" => rpc_health_json_action(args),
        "--peer-health" => Ok(CliAction::Print(peer_health_text(args)?)),
        "--peer-health-json" => peer_health_json_action(args),
        "--mempool-status" => Ok(CliAction::Print(mempool_status_text(args)?)),
        "--mempool-status-json" => mempool_status_json_action(args),
        ...
        "--check-config-drift" => check_config_drift_action(args),
        "--check-config-drift-json" => check_config_drift_json_action(args),
        "--reconcile-node-config" => reconcile_node_config_action(args),
        "--reconcile-node-config-json" => reconcile_node_config_json_action(args),
        ...
        "--fleet-health" => fleet_health_action(args),
        "--fleet-health-json" => fleet_health_json_action(args),
        "--node-health" => node_health_action(args),
        "--node-health-json" => node_health_json_action(args),
        "--node-list" => node_list_action(args),
        "--node-list-json" => node_list_json_action(args),
        "--node-status" => node_status_action(args),
        "--node-status-json" => node_status_json_action(args),
        "--list-api-tokens" => list_api_tokens_action(args),
        "--list-api-tokens-json" => list_api_tokens_json_action(args),
        "--create-api-token" => create_api_token_action(args),
        "--revoke-api-token" => revoke_api_token_action(args),
