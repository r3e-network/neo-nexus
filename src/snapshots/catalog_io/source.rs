...
use super::http::fetch_https_response;

pub(in crate::snapshots) enum SnapshotCatalogSource {
    Https(Url),
    File(PathBuf),
}

impl SnapshotCatalogSource {
    pub(in crate::snapshots) fn parse(value: &str, label: &str) -> Result<Self> {
...
pub(in crate::snapshots) fn read_catalog_source_bytes(
    source: SnapshotCatalogSource,
    max_bytes: u64,
    label: &str,
) -> Result<Vec<u8>> {
    match source {
        SnapshotCatalogSource::Https(url) => {
        ...
        SnapshotCatalogSource::File(path) => {
...
pub(in crate::snapshots) fn is_https_source(value: &str) -> bool {
    value.trim().starts_with("https://")
}

pub(in crate::snapshots) fn optional_trimmed(value: &Option<String>) -> Option<&str> {
    value
        .as_deref()
        .map(str::trim)
